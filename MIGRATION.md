...
- **§2.3/§8.1 PublishTrimmed 验证**：ExportSmokeTest 以 PublishTrimmed+单文件自包含发布后运行全量回归通过；但后续实测**打包应用开启裁剪后出现启动崩溃（combase.dll 0x802B000A）**，最终决定 **PublishTrimmed=False（关闭裁剪）**，保留 PublishReadyToRun=True。
...
- **坑 16（新增，美化）**：CommunityToolkit DataGrid 默认选中/编辑高亮是银灰旧样式；可在 App.xaml 覆盖其主题资源实现现代化（ThemeResource 机制，应用层优先）：行选中 `DataGridRowSelectedBackgroundColor`+`DataGridRowSelectedBackgroundOpacity`（及 Hovered/Unfocused 变体）、悬停 `DataGridRowHoveredBackgroundColor`、表头悬停/按下 `DataGridColumnHeaderHoveredBackgroundColor`/`DataGridColumnHeaderPressedBackgroundColor`、编辑焦点框 `DataGridCellFocusVisualPrimaryBrush`/`DataGridCellFocusVisualSecondaryBrush`。注意：WinUI 3 XAML 里 `Double` 必须写 `x:Double`；已按亮/暗两套主题配置（Fluent 配色：主题蓝浅染 + 中性灰）。**编辑态文本框的"黑色包边"**是 WinUI 3 文本框聚焦边框默认纯黑（`TextControlBorderBrushFocused`），同样在 App.xaml 覆盖为主题蓝即可（该键定义在 XamlControlsResources 的 ThemeDictionaries 里，外层 ThemeDictionaries 优先级更高）。
- **坑 17（新增，图标）**：任务栏图标不对有两个原因——① `Assets\*.png` 还是模板占位图（打包运行时任务栏用包清单 Square44x44Logo）；② `AppWindow.SetIcon` 相对路径在打包运行时不可靠，须用 `Path.Combine(AppContext.BaseDirectory, "Assets\\app.ico")`。已做：从 `app.ico` 提取 256×256 PNG 母版重生成全套 PNG（88/24/300/620×300/1240×600/48/50，透明画布居中），csproj 加 `<ApplicationIcon>Assets\app.ico</ApplicationIcon>`（exe 图标）。**注**：System.Drawing 的 `Icon(path,64,64).ToBitmap()` 对 PNG 条目会抛异常，需手写 ICO 目录解析直接抽取 PNG 数据（本会话已实现，勿再走 ToBitmap 老路）。
- **坑 18（新增，编辑态黑色包边）**：DataGridCell 默认模板里有 `FocusVisual` 矩形，编辑/聚焦时以 `BorderBrush`（默认黑）描边显示黑色包边；覆盖 `TextControlBorderBrushFocused` 无效。**根治**：用 `DataGrid.CellStyle` 重写 DataGridCell 模板（只留 ContentPresenter，无任何边框/焦点元素）+ `BorderThickness=0`；编辑文本框去边框用两招——模板列 CellEditingTemplate 的 TextBox 直接 `BorderThickness="0" Background="Transparent"`，文本列用 `EditingElementStyle="{StaticResource BorderlessTextBoxStyle}"`（DataGridBoundColumn.EditingElementStyle 存在）。共享样式 `BorderlessDataGridCellStyle`/`BorderlessTextBoxStyle` 定义在 App.xaml（TargetType 分别为 `dg:DataGridCell` 与 `TextBox`）。行选中/休息日高亮在行级，不受单元格重模板影响。
- **坑 19（新增，坑18 的连锁）**：极简单元格模板的两个必配项——① `HorizontalContentAlignment=Stretch`（否则编辑 TextBox 按内容收缩、显示为窄框；Stretch 后撑满列宽并随窗口缩放）；② ContentPresenter 设 `Background="Transparent"`（否则**空文本单元格无命中区域，点击穿透无法选中进入编辑态**——实测只有点有内容的单元格才能编辑）。
- **坑 20（新增，编辑框外观不统一）**：`DataGridTextColumn` 内置编辑框（走 `EditingElementStyle`）与模板列自定义编辑框外观不一致（前者方形、后者圆角）。**根治**：可编辑文本列一律改 `DataGridTemplateColumn`（CellTemplate=TextBlock 显示 + CellEditingTemplate=TextBox 编辑，BorderThickness=0、Background=Transparent，与多行列同款 XAML），外观由同一套模板保证一致；`BorderlessTextBoxStyle`/`EditingElementStyle` 已废弃删除。星期列宽度 60→80。
